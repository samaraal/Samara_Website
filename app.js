...
function ensureCareerGenderField(form) {
  if (!form || form.elements.gender) return form?.elements.gender || null;
  const title = form.elements.title;
  if (!title) return null;
  const titleBlock = title.closest("label") || title.parentElement;
  if (!titleBlock) return null;

  const isLabel = titleBlock.tagName === "LABEL";
...
function updateCareerSalutations(form) {
  ...
  title.innerHTML = '<option value="">Select salutation</option>' + choices.map(value => `<option value="${value}">${value}</option>`).join("");
  title.disabled = !gender;
  if (previous && choices.includes(previous)) title.value = previous;
}

function validateCareerContactsAndSalutation(form, showPopup = true) {
  ...
  if (mobile && emergency && mobile === emergency) {
    const message = "Applicant mobile number and Emergency / Parent contact number must be different. Please provide another contact number for emergencies.";
...
if (careerForm) {
  ...
  careerForm.elements.emergency_contact?.addEventListener('input', () => {
    ...
    if(mobile.length===10&&emergency.length===10&&mobile===emergency){
      const message='Applicant mobile number and Emergency / Parent contact number must be different.';
      careerForm.elements.emergency_contact.setCustomValidity(message);
    }
  });
  careerForm.elements.emergency_contact?.addEventListener('blur', () => {
    const result=validateCareerContactsAndSalutation(careerForm,false);
    if(!result.ok && normaliseCareerPhone(careerForm.elements.emergency_contact?.value).length===10) window.alert(result.message);
  });
...
document.querySelector("#career-form")?.addEventListener("submit", async event => {
  ...
  const contactCheck = validateCareerContactsAndSalutation(form,true);
  if (!contactCheck.ok) {
    status.classList.add("error");
    status.textContent = contactCheck.message;
    return;
  }
